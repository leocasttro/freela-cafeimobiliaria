<template>
  <CardWrapper>
    <div class="card mt-3 card-custon">
      <Carousel>
        <Slide v-for="slide in cardData" :key="slide.id">
          <img :src="slide.link" class="card-img-top card-img-custom" style="object-fit: cover" />
        </Slide>
      ...
      <div class="card-body p-1">
        ...
        <div class="card-text m-2 mt-5" style="color: gray">
...
const loadProperty = async () => {
  try {
    const response = await fetch(`../../public/jsons/imoveis/${props.cod}.json`)
    if (!response.ok) {
      throw new Error('Erro ao carregar o arquivo JSON')
    }
    const data = await response.json()
    cardData.value = data.imagens
  } catch (error) {
    console.error(error)
  }
}
